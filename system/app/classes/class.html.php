<?php
	...
	*/
	class Html
	{
		...
		public static function pageHKN()
		{
			global $dbh, $config, $lang, $hotel, $version;
			if(isset($_GET['url']))
			{
				$pageHKN = filter($_GET['url']);
				$allowed = array();
				foreach (new DirectoryIterator(L) as $file) {
					$file = explode('.php', $file);
					$allowed[] = $file[0];
				}
				if($pageHKN)
				{
					...
					$fileExists = L ."{$pageHKN}.php";
					if(file_exists(filter($fileExists)))
					{
						$content = in_array($pageHKN, $allowed) ? include(L ."/{$pageHKN}.php") : '';
					}
					else
					{
						include L . "/404.php";
				...
				{
					include L . "/dashboard.php";
				}
			}
			else
			{
				include L . "dashboard.php";
				header('Location: '.$config['hotelUrl'].'/housekeeping/dashboard.php');
			}
			switch($pageHKN)
			{
				case $pageHKN:
